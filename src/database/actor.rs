...
impl Handler<NewEmailMessage> for DatabaseActor {
    ...
    fn handle(&mut self, msg: NewEmailMessage, _ctx: &mut Context<Self>) -> Self::Result {
        log::trace!("Database actor received new email to insert into database");
        ...
        futures::executor::block_on(async {
            database.use_db("mail").await.expect("Failed to change to mail database. A malfunctioning database is not recoverable.");
